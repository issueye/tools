<template>
        ...
        <el-card class="form-box">
            <el-form :model="form" label-width="120px">
                <el-form-item label="检索" :inline="true">
                    ...
                    <el-col :span="4">
                        <span class="form-button"><el-button type="primary" @click="handleQuery">查询</el-button></span>
                        <span class="form-button"><el-button type="primary" @click="handleAdd">添加</el-button></span>
                    </el-col>
                </el-form-item>
            </el-form>
        </el-card>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="30%" top="1vh" :close-on-click-modal="false"
        ...
        <el-form :model="editForm" :label-position="right" label-width="80px">
            ...
            <el-form-item label="日志等级">
                <el-input v-model.number="editForm.level" />
            </el-form-item>
...
// 获取参数列表
const getData = async () => {
    let { data } = await apiMonitorList(form);
...
const handleEnable = async (row) => {
    let { data } = await apiMonitorModifyState(row.id)
...
const handleClose = (row) => {
    ElMessageBox.confirm(
        ...
        // 调用接口
        let { data } = await apiMonitorDel(row.id)
        if (data.code == 200) {
            ElMessage({
                type: 'success',
                message: data.message,
...
const handelSave = async () => {
    if (dialogTitle.value == '添加') {
        let { data } = await apiMonitorAdd(editForm)
        if (data.code == 200) {
            ElMessage({
                type: 'success',
                message: data.message,
    ...
    if (dialogTitle.value == '修改') {
        let { data } = await apiMonitorModify(editForm)
        if (data.code == 200) {
            ElMessage({
                type: 'success',
                message: data.message,
